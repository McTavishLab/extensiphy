...
def main():
    ...
    test_vcffixer(hand_checked_vcf, hand_checked_align, output_file)

    check_test_output(hand_checked_align, output_file)

def test_vcffixer(vcf_, align_, output_):
    ...
    vfix = subprocess.Popen(["../../../modules/vcffixer.py", "--vcf_file", vcf_, "--align_file", align_, "--out_file", output_], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
