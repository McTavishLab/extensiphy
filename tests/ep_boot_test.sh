...
../extensiphy.sh -u PHYLO -b ON -a ../testdata/combo.fas -t ../testdata/combo.tre -d ../testdata -1 _R1.fq -2 _R2.fq -o ep_test_boot >> logfile.txt 2>&1

ALIGN=./ep_test_boot/RESULTS/extended.aln
PHYLO=./ep_test_boot/RESULTS/RAxML_bestTree.consensusFULL
BOOT=./ep_test_boot/RESULTS/RAxML_bipartitions.majority_rule_bootstrap_consensus
num_lines=$(wc -l ./ep_test_boot/RESULTS/extended.aln)
num_seqs=$(grep -c ">" ./ep_test_boot/RESULTS/extended.aln)
check_tree=$(grep -c ":0.0;" ./ep_test_boot/RESULTS/RAxML_bestTree.consensusFULL)
check_boot=$(grep -c "taxon_17" ./ep_test_boot/RESULTS/RAxML_bipartitions.majority_rule_bootstrap_consensus)
